...
import com.bishang366.adapter.TrustAdapterNewHistory;
...
import com.bishang366.ui.dialog.EntrustOperateDialogFragment;
import com.bishang366.ui.option.OptionOrderAdapter;
 ...
 * 当前委托的界面
 */
public class TrustOptionListActivity extends BaseActivity implements View.OnClickListener, ITrustOption.View {

    public static void show(Activity activity,String symbol) {
        Intent intent = new Intent(activity, TrustOptionListActivity.class);
...
//    ViewPager vp_trust;
    ...
    TextView tv_confirm;
    private List<OptionDetailInfo> entrusts = new ArrayList<>();
    OptionOrderAdapter adapter_2;
    private List<OptionDetailInfo> entrustHistories = new ArrayList<>();
    OptionOrderAdapter adapter;
    private int currentPage = 1;
    private int historyPage = 1;
    private ITrustOption.Presenter mPresenter;
    ...
    protected void initViews(Bundle savedInstanceState) {
        ...
        mPresenter = new TrustOptionPresentImpl(this);
...
//        initViewPager();
    ...
    void countDown() {
        ...
        timer.schedule(new TimerTask() {
            ...
            @Override
            public void run() {
                tv_confirm.post(new Runnable() {
                    @Override
                    public void run() {
                        if (adapter_2 != null && adapter_2.getData().size() > 0){
                            long c = System.currentTimeMillis();
                            for (int i = 0; i < adapter_2.getData().size(); i++) {
                                OptionDetailInfo optionDetailInfo = adapter_2.getData().get(i);
                                long closeTime = WonderfulDateUtils.getTimeMillis("", optionDetailInfo.getCloseTime());
                                int l = (int) ((closeTime - c) / 1000);
                                if (l <= 0 || l > optionDetailInfo.getCycleLength()){
                                    adapter_2.getData().remove(optionDetailInfo);
                                }else {
                                    optionDetailInfo.setCountDownTime(l);
                                }
...
//    }
    ...
    @Override
    public void getHistorySuccess(List<OptionDetailInfo> entrustHistories) {
        hideLoadingPopup();
        if (refreshLayout == null) {
            return;
        }
    ...
    public void setListData(List<OptionDetailInfo> list) {
        if (list != null && list.size() > 0) {
            ...
            if (isCurrent) {
                if (currentPage == 1) {
                    cancelCountDown();
                    entrusts.clear();
                    adapter_2 =  new OptionOrderAdapter(R.layout.adapter_option_history, list, false);
                    rvAds.setLayoutManager(new LinearLayoutManager(TrustOptionListActivity.this));
                ...
                if (historyPage == 1) {
                    ...
                    adapter = new OptionOrderAdapter(R.layout.adapter_option_history, list, true);
                    rvAds.setLayoutManager(new LinearLayoutManager(TrustOptionListActivity.this));
    ...
    @Override
    public void getCurrentSuccess(List<OptionDetailInfo> entrust) {
        hideLoadingPopup();
        if (refreshLayout == null) {
            return;
        }
